...
function detect_timezone() {
	...
	printf "\n==> The system detected your timezone to be: %s\n" "${TIMEZONE}" &>>"${LOGFILE}"
}
...
function change_timezone() {
	...
	printf "\n==> Your timezone will be set to: %s\n" "${TIMEZONE}" &>>"${LOGFILE}"
	return 0
}
...
# NOTE: Showing a progress gauge in whiptail is a pain.
# I do it by passing the name of the process to measure (a function name) to a calling function.
# The calling function calls the function and sends it to the background, and then immediately captures its PID.
# We check for that PID and keep showing the progress meter if its still in the PID table.
# If it drops out of the PID table, then we immediately show 98 99 100 pct progress with a 3 second wait between each.
# If the process is taking a very long time, we show 97pct 98pct 97pct 98pct until the PID drops out of the PID table.
# This way the user never suspects the install has frozen, just that he's going spastic.
...
function special_progress_gauge() {
	...
	printf "=== Watching PID %s for progress ===\n\n" "${thepid}" &>>"${LOGFILE}"
	num=10 # Shortest progress bar could be 10 sec to 30 sec
	while true; do
		show_progress 1 "${num}" 1 3
		...
		while ps aux | grep -v 'grep' | grep "${thepid}" &>/dev/null; do
			...
			show_progress "${num}" $((num + 1))
			num=$((num + 1))
		done
		show_progress 99 100 3 3 # If we have completion, we add 6 sec. Max of 53 sec.
		printf "\n=== No longer watching PID: %s ===\n" "${thepid}" &>>"${LOGFILE}"
		break
	done | whiptail --backtitle "${backmessage}" --title "Progress Gauge" --gauge "\n${message}" 10 70 0
}
...
function auto_partitions_noswap() {
	...
	if efi_boot_mode; then
		...
		if [[ ${DISKTABLE} == GPT ]]; then
			...
			print_step "Partitioning the disk" &>>"${LOGFILE}"
			echo 'type=83, bootable' | sfdisk "${DISK}" &>>"${LOGFILE}"
			reread_partition_table
...
function format_partitions() {
	...
	print_step "Formatting the partitions" &>>"${LOGFILE}"
	...
	for PART in ${PARTITIONS}; do
		...
		"${BIOS_BOOT_PARTITION}")
			...
			;;
		"${EFI_PARTITION}")
			echo "[*] Formatting the EFI system partition as fat32..." &>>"${LOGFILE}"
			mkfs.fat -F 32 "${PART}" &>>"${LOGFILE}"
			;;
		"${SWAP_PARTITION}")
			echo "[*] Formatting the swap partition..." &>>"${LOGFILE}"
			mkswap "${PART}" &>>"${LOGFILE}"
			swapon "${PART}" &>>"${LOGFILE}"
			;;
		"${ROOT_PARTITION}")
			echo "[*] Formatting the root partition as ${FILESYSTEM}..." &>>"${LOGFILE}"
			if [[ ${FILESYSTEM} == btrfs ]]; then
				mkfs.btrfs -f "${PART}" &>>"${LOGFILE}"
			elif [[ ${FILESYSTEM} == ext4 ]]; then
				mkfs.ext4 -F "${PART}" &>>"${LOGFILE}"
			fi
			;;
		*) TERM=ansi whiptail --backtitle "${backmessage}" --title "ERROR!" \
			--infobox "Bad disk format request.\nCan't make that disk * * format." 8 0 && sleep 5 && startmenu "D" ;;
		esac
	done
	return 0
}
...
function mount_filesystems() {
	print_step "Mounting the file systems" &>>"${LOGFILE}"

	# Mount the root partition
	if [[ ${FILESYSTEM} == btrfs ]]; then
		mount -t btrfs "${ROOT_PARTITION}" /mnt &>>"${LOGFILE}"
		subvolumes_setup
	elif [[ ${FILESYSTEM} == ext4 ]]; then
		mount -t ext4 "${ROOT_PARTITION}" /mnt &>>"${LOGFILE}"
	fi

	# Mount the EFI System partition
	if efi_boot_mode; then
		mount --mkdir "${EFI_PARTITION}" /mnt/boot &>>"${LOGFILE}"
	fi
}

# CREATE BTRFS SUBVOLUMES
function create_subvolumes() {
	echo "[*] Creating subvolumes for root, home, .snapshots, swap, tmp and var/log directories..." &>>"${LOGFILE}"
	{
	...
	if [[ -n ${SWAPFILE_SIZE} ]]; then
		btrfs subvolume create /mnt/@swap &>>"${LOGFILE}"
	fi
	btrfs subvolume create /mnt/@tmp &>>"${LOGFILE}"
	btrfs subvolume create /mnt/@var_log &>>"${LOGFILE}"
}

# MOUNT ALL BTRFS SUBVOLUMES
function mount_subvolumes() {
	echo "[*] Mounting the subvolumes..." &>>"${LOGFILE}"
	{
	...
	if [[ ${SWAPFILE} == true ]]; then
		mkdir -p /mnt/swap &>>"${LOGFILE}"
		mount -o "${BTRFS_MOUNT_OPTIONS},subvol=@swap" "${ROOT_PARTITION}" /mnt/swap &>>"${LOGFILE}"
	fi
	mount -o "${BTRFS_MOUNT_OPTIONS},subvol=@tmp" "${ROOT_PARTITION}" /mnt/tmp &>>"${LOGFILE}"
	mount -o "${BTRFS_MOUNT_OPTIONS},subvol=@var_log" "${ROOT_PARTITION}" /mnt/var/log &>>"${LOGFILE}"
}
...
function create_swap_file() {
	print_step "Creating a swap file" &>>"${LOGFILE}"
...
function install_base() {
	pacstrap -K /mnt "${base_pkgs[@]}" &>>"${LOGFILE}"
}

# MICROCODE INSTALL
function install_microcode() {
	print_step "Installing microcode update image" &>>"${LOGFILE}"
	# Determine processor type and install microcode
	PROC_TYPE=$(lscpu)
	TERM=ansi whiptail --backtitle "${backmessage}" --title "Microcode" \
		--infobox "Installing microcode update image..." 10 70
	sleep 3
	if grep -Eiq "GenuineIntel" <<<"${PROC_TYPE}"; then
		arch-chroot /mnt pacman -S --noconfirm --needed intel-ucode &>>"${LOGFILE}"
	elif grep -Eiq "AuthenticAMD" <<<"${PROC_TYPE}"; then
		arch-chroot /mnt pacman -S --noconfirm --needed amd-ucode &>>"${LOGFILE}"
	fi
	TERM=ansi whiptail --backtitle "${backmessage}" --title "Microcode" \
		--infobox "Microcode successfully installed..." 10 70
	sleep 3
}
...
function install_graphics_drivers() {
	...
	*)
	...
	print_step "Installing graphics drivers" &>>"${LOGFILE}"
	TERM=ansi whiptail --backtitle "${backmessage}" --title "Graphics Drivers" \
		--infobox "Installing graphics drivers..." 10 70
	sleep 3
	if arch-chroot /mnt pacman -S --noconfirm --needed "${graphics_drivers[@]}" &>>"${LOGFILE}"; then
		TERM=ansi whiptail --backtitle "${backmessage}" --title "Graphics Drivers" \
			--infobox "Graphics drivers successfully installed." 10 70
		sleep 3
	fi
}

function install_input_drivers() {
	print_step "Installing input drivers" &>>"${LOGFILE}"
	# Input drivers install
	TERM=ansi whiptail --backtitle "${backmessage}" --title "Input Drivers" \
		--infobox "Installing input drivers..." 10 70
	sleep 3
	if arch-chroot /mnt pacman -S --noconfirm --needed "${input_drivers[@]}" &>>"${LOGFILE}"; then
		TERM=ansi whiptail --backtitle "${backmessage}" --title "Input Drivers" \
			--infobox "Input drivers successfully installed." 10 70
		sleep 3
	fi
}
...
function install_wifi_drivers() {
	...
	if grep -Eiq "BCM43" <<<"${WIFI_CARD}"; then
		print_step "Installing wifi drivers" &>>"${LOGFILE}"
		TERM=ansi whiptail --backtitle "${backmessage}" --title "Wifi Drivers" \
			--infobox "Installing wifi drivers..." 10 70
		sleep 3
		if arch-chroot /mnt pacman -S --noconfirm --needed "${wifi_drivers[@]}" &>>"${LOGFILE}"; then
			TERM=ansi whiptail --backtitle "${backmessage}" --title "Wifi Drivers" \
				--infobox "Wifi drivers successfully installed." 10 70
			sleep 3
		fi
	fi
}
...
function install_essential_pkgs() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${essential_pkgs[@]}" &>>"${LOGFILE}"
}
function install_xorg() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${xorg_pkgs[@]}" &>>"${LOGFILE}"
}
function install_networking_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${networking_pkgs[@]}" &>>"${LOGFILE}"
}
function install_sound_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${sound_pkgs[@]}" &>>"${LOGFILE}"
}
function install_bluetooth_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${bluetooth_pkgs[@]}" &>>"${LOGFILE}"
}
function install_archive_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${archive_pkgs[@]}" &>>"${LOGFILE}"
}
function install_disk_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${disk_pkgs[@]}" &>>"${LOGFILE}"
}
function install_printing_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${printing_pkgs[@]}" &>>"${LOGFILE}"
}
function install_laptop_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${laptop_pkgs[@]}" &>>"${LOGFILE}"
}
function install_fonts() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${font_pkgs[@]}" &>>"${LOGFILE}"
}
function install_pacman_software() {
	arch-chroot /mnt pacman -S --noconfirm --needed "${pacman_pkgs[@]}" &>>"${LOGFILE}"
}

# INSTALL DESKTOP ENVIRONMENT
function install_desktop() {
	DESKTOP=("${mydesktop[@]}")
	arch-chroot /mnt pacman -S --noconfirm --needed "${DESKTOP[@]}" &>>"${LOGFILE}"
}

# LOGIN DISPLAY MANAGER
function enable_display_manager() {
	# Enable login display manager
	print_step "Enabling login display manager" &>>"${LOGFILE}"
	arch-chroot /mnt systemctl enable "${display_mgr}.service" &>>"${LOGFILE}"
}

# ENABLE SERVICES
function enable_services() {
	print_step "Enabling essential services" &>>"${LOGFILE}"
	...
	for service in "${my_services[@]}"; do
		case ${service} in
		"dhcpcd")
			arch-chroot /mnt systemctl disable "${service}.service" &>>"${LOGFILE}"
			;;
		"ntpd")
			arch-chroot /mnt ntpd -qg &>>"${LOGFILE}"
			arch-chroot /mnt systemctl enable "${service}.service" &>>"${LOGFILE}"
			;;
		"reflector")
			arch-chroot /mnt systemctl enable "${service}.timer" &>>"${LOGFILE}"
			arch-chroot /mnt systemctl enable "${service}.service" &>>"${LOGFILE}"
			;;
		*)
			arch-chroot /mnt systemctl enable "${service}.service" &>>"${LOGFILE}"
...
function install_aur_helper() {
	...
	if whiptail --backtitle "${backmessage}" --title "AUR Helper" \
		...
		print_step "Installing '${AUR_HELPER}' AUR helper" &>>"${LOGFILE}"
		sed -i 's/^[#[:space:]]*%wheel/%wheel/g' /mnt/etc/sudoers
		execute_user "${SUDO_USER}" "cd /home/${SUDO_USER}/ && git clone https://aur.archlinux.org/${AUR_HELPER}.git && (cd ${AUR_HELPER} && makepkg -si --noconfirm --needed) && rm -rf ${AUR_HELPER}" &>>"${LOGFILE}"
...
function install_grub() {
	print_step "Installing GRUB boot loader" &>>"${LOGFILE}"
	TERM=ansi whiptail --backtitle "${backmessage}" --title "Boot loader" \
		--infobox "Installing GRUB boot loader..." 10 70
	sleep 2
	arch-chroot /mnt pacman -S --noconfirm --needed grub os-prober &>>"${LOGFILE}"

	# Grub gets installed differently on efi & non-efi systems
	if efi_boot_mode; then
		arch-chroot /mnt pacman -S --noconfirm --needed efibootmgr dosfstools &>>"${LOGFILE}"
		if arch-chroot /mnt grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id=GRUB &>>"${LOGFILE}"; then
			TERM=ansi whiptail --backtitle "${backmessage}" --title "Boot loader" \
				--infobox "GRUB boot loader successfully installed." 10 70

			sleep 2
		fi
	else
		if arch-chroot /mnt grub-install --target=i386-pc "${DISK}" &>>"${LOGFILE}"; then
			TERM=ansi whiptail --backtitle "${backmessage}" --title "Boot loader" \
				--infobox "GRUB boot loader successfully installed." 10 70
	...
	# How did we do?
	whiptail --backtitle "${backmessage}" --title "Log File" \
		--textbox /tmp/install.log --scrolltext 30 100
}
...
function pacman_configuration_chroot() {
	...
	# Update database
	arch-chroot /mnt pacman -Sy &>>"${LOGFILE}"
	sleep 2
}

# EDIT MKINITCPIO CONFIGURATION FILE
function mkinitcpio_conf() {
	print_step "Initramfs" &>>"${LOGFILE}"
	TERM=ansi whiptail --backtitle "${backmessage}" --title "System Configuration" \
		--infobox "Modifying /etc/mkinitcpio.conf..." 10 70
...
function hostname_setup() {
	...
	echo "${HOSTNAME}" | tee /mnt/etc/hostname &>>"${LOGFILE}"

	tee /mnt/etc/hosts &>>"${LOGFILE}" <<HOSTS
127.0.0.1      localhost
::1            localhost
...
function root_password() {
	if PASSWORD1=$(whiptail --backtitle "${backmessage}" --title "Root Password" \
		...
		if [[ ${PASSWORD1} == "${PASSWORD2}" ]]; then
			...
			printf "%s\n%s\n" "${ROOT_PASSWORD}" "${ROOT_PASSWORD}" | arch-chroot /mnt passwd &>>"${LOGFILE}"
			#echo -e "${ROOT_PASSWORD}\n${ROOT_PASSWORD}" | arch-chroot /mnt passwd &>>"${LOGFILE}"
		else
...
function user_password() {
	if PASSWORD1=$(whiptail --backtitle "${backmessage}" --title "User Password" \
		...
		if [[ ${PASSWORD1} == "${PASSWORD2}" ]]; then
			...
			printf "%s\n%s\n" "${USER_PASSWORD}" "${USER_PASSWORD}" | arch-chroot /mnt passwd "${SUDO_USER}" &>>"${LOGFILE}"
			#echo -e "${USER_PASSWORD}\n${USER_PASSWORD}" | arch-chroot /mnt passwd "${SUDO_USER}" &>>"${LOGFILE}"
		else
...
function add_user_account() {
	...
	arch-chroot /mnt useradd -m -g users -G wheel,audio,video,network,storage,rfkill -s /bin/bash "${SUDO_USER}" &>>"${LOGFILE}"
...
function decrease_swappiness() {
	if [[ ${SWAP} == true || ${SWAPFILE} == true ]]; then
		print_step "Decreasing swappiness value" &>>"${LOGFILE}"
		echo "vm.swappiness=10" | tee -a /mnt/etc/sysctl.d/99-swappiness.conf &>>"${LOGFILE}"
	fi
}

# XORG KEYBOARD CONFIGURATION
function xorg_keyboard_configuration() {
	print_step "Xorg/Keyboard configuration" &>>"${LOGFILE}"
	mkdir -p /mnt/etc/X11/xorg.conf.d &>>"${LOGFILE}"
	tee /mnt/etc/X11/xorg.conf.d/00-keyboard.conf &>>"${LOGFILE}" <<EOF
...
function startmenu() {
	while true; do
		if menupick=$(
			...
			"B")
				print_step "Installing the base system" &>>"${LOGFILE}"
			...
			"U")
				print_step "User management" &>>"${LOGFILE}"
			...
			"S")
				...
				install_microcode
				install_drivers
				print_step "Installing Xorg server" &>>"${LOGFILE}"
				special_progress_gauge install_xorg "Installing Xorg server..."
				print_step "Installing networking software" &>>"${LOGFILE}"
				special_progress_gauge install_networking_software "Installing networking software..."
				print_step "Installing sound software" &>>"${LOGFILE}"
				special_progress_gauge install_sound_software "Installing multimedia software..."
				print_step "Installing bluetooth software" &>>"${LOGFILE}"
				special_progress_gauge install_bluetooth_software "Installing bluetooth software..."
				print_step "Installing archiving & compression software" &>>"${LOGFILE}"
				special_progress_gauge install_archive_software "Installing archiving & compression software..."
				print_step "Installing disk software" &>>"${LOGFILE}"
				special_progress_gauge install_disk_software "Installing disk software..."
				print_step "Installing printing software" &>>"${LOGFILE}"
				special_progress_gauge install_printing_software "Installing printing software..."
				print_step "Installing laptop software" &>>"${LOGFILE}"
				special_progress_gauge install_laptop_software "Installing laptop software..."
				print_step "Installing fonts" &>>"${LOGFILE}"
				special_progress_gauge install_fonts "Installing fonts..."
				print_step "Installing more parcman software" &>>"${LOGFILE}"
				special_progress_gauge install_pacman_software "Installing more pacman software..."
				print_step "Installing '${desktop_choice}' desktop environment" &>>"${LOGFILE}"
				special_progress_gauge install_desktop "Installing '${desktop_choice}' desktop environment..."
				whiptail --backtitle "${backmessage}" --title "Software Installation" \
					--msgbox "Xorg server and desktop environment have been installed.\nOK to continue." 10 70
				whiptail --backtitle "${backmessage}" --title "Installation Progress" \
					--textbox /tmp/install.log --scrolltext 30 100
